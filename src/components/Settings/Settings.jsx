...
const Settings = () => {
  ...
  return (
    <>
      ...
      `}</style>
      <div className="min-h-screen bg-[#e6f8ef] p-4 sm:p-6 lg:p-8 text-[#3f6870] animate-fade-in">
        <Toaster position="top-center" />
        <div className="bg-[#e6f8ef]/80 backdrop-blur-sm rounded-xl p-4 sm:p-6 lg:p-8 max-w-4xl mx-auto border border-[#3f6870]/50 shadow-xl">
          <div className="mb-6 text-center sm:mb-8">
            <img
              src={user?.img || userProfile}
              alt="Profile"
              className="mx-auto rounded-full w-24 h-24 sm:w-28 sm:h-28 border-4 border-[#07A658] shadow-md animate-image-hover"
            />
            <h2 className="text-[#142528] text-2xl sm:text-3xl font-bold mt-3">
                  ...
                  {({ selected }) => (
                    <button
                      className={`text-sm sm:text-base font-semibold pb-2 transition duration-300 ${
                        selected
                          ? "text-[#142528] border-b-2 border-[#07A658] tab-indicator"
                          : "text-[#3f6870] hover:text-[#142528] hover:border-b-2 hover:border-[#07A658]/80"
                      }`}
                    >
                      {tab}
                    </button>
              ...
              <Tab.Panel className="animate-fade-in-panel">
                <form onSubmit={handleProfileSubmit} className="grid grid-cols-1 gap-4 lg:grid-cols-2 sm:gap-6">
                  {["firstName", "last_Name", "Username", "email", "Phone", "country"].map((field) => (
                    <div key={field}>
                      <label className="block text-sm sm:text-base font-medium mb-1 capitalize text-[#3f6870]">
                        {field}
                      </label>
                      <input
                        name={field}
                        ...
                        className="w-full px-4 py-2 sm:py-3 rounded-xl bg-[#e6f8ef] border border-[#3f6870]/50 text-[#3f6870] placeholder-[#3f6870]/70 hover:border-[#07A658]/80 focus:ring-2 focus:ring-[#07A658] focus:border-[#07A658] outline-none transition-all duration-300"
                  ...
                  <div>
                    <label className="block text-sm sm:text-base font-medium mb-1 text-[#3f6870]">
                      Plan
                    </label>
                    <input
                      name="Plan"
                  ...
                  <div>
                    ...
                    <input
                      ...
                      value={user?.dateOc}
                      readOnly
                      className="w-full px-4 py-2 sm:py-3 rounded-xl bg-[#e6f8ef] border border-[#3f6870]/50 text-[#3f6870] cursor-not-allowed"
                    />
                  </div>
              ...
              <Tab.Panel className="animate-fade-in-panel">
                <form onSubmit={handlePasswordSubmit} className="space-y-4 sm:space-y-6">
                  <div className="relative">
                    <label className="block mb-1 text-sm sm:text-base font-medium text-[#3f6870]">
                      Old Password
                    </label>
                    <input
                    ...
                    <span
                      ...
                      className="absolute right-3 top-9 sm:top-10 text-[#3f6870] cursor-pointer"
                    >
