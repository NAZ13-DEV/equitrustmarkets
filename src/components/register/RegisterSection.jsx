  ...
  const handleSubmitCheck = async (e) => {
    ...
    if (Object.keys(errors).length > 0) {
      if (allFieldsFilled && phoneEmpty) {
        toast.error(
          'Please fill in the country field properly By clicking to select country',
        );
      } else {
    ...
    <div className='flex items-center justify-center px-4 py-12 bg-white'>
      ...
      <div className='grid w-full max-w-6xl grid-cols-1 my-6 overflow-hidden bg-white shadow-xl rounded-3xl md:grid-cols-2'>
        ...
        <div className='bg-gradient-to-r from-[#007A5E] to-[#00C897] text-white p-10 flex flex-col justify-center space-y-6 animate-slide-in-left'>
          <h1 className='text-2xl font-extrabold leading-snug md:text-4xl'>
            Join Optima Trade Market
          </h1>
          <p className='text-lg opacity-90'>
            The smartest way to trade globally. Enjoy institutional-grade tools,
            24/7 support, and secure asset control.
          </p>
        ...
        <div className='relative p-10'>
            ...
            {[1, 2].map((s) => (
              <button
                ...
                className={`w-10 h-10 rounded-full border-2 transition duration-300 flex items-center justify-center text-sm font-bold
                  ${
                    step === s
                      ? 'bg-[#07A658] text-white border-[#07A658] shadow-md shadow-green-300'
                      : 'border-[#07A658] text-[#07A658] hover:bg-[#e6f8ef]'
                  }`}
              >
                {s}
              </button>
            ...
            {step === 1 && (
              <div className='space-y-4 animate-fade-in'>
                {['username', 'firstName', 'last_Name'].map((field) => (
                  <div key={field}>
                    <label className='block text-sm text-[#3f6870] capitalize mb-1'>
                      {field.replace('_', ' ')}
                    </label>
                    <input
                      name={field}
                ...
                <div>
                  <label className='block text-sm text-[#3f6870] mb-1'>
                    Country
                  </label>
                  <input
                    type='text'
          ...
          <p className='mt-8 text-sm text-center text-[#3f6870]'>
            ...
            <Link
              to='/login'
              className='text-[#07A658] font-semibold hover:underline'
            >
              Login
            </Link>
          </p>
